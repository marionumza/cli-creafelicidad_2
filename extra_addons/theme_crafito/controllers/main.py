...
class Web_Editor(Web_Editor):
    ...
    def save_secondary_scss(self, url, bundle_xmlid, content):
        IrAttachment = request.env["ir.attachment"]
        if url != '/theme_crafito/static/src/scss/colors/color_picker_sec.scss':
            return super(Web_Editor, self).save_scss(url, bundle_xmlid, content)

        datas = base64.b64encode((content or "\n").encode("utf-8"))
        custom_url = '/theme_crafito/static/src/scss/colors/color_picker_sec' + \
            str(request.website.id)+'.scss'
...
class CrafitoEcommerceShop(WebsiteSale):
    ...
    # ==============================
            ...
            return super(CrafitoEcommerceShop, self).shop(page=page, category=category, search=search, **post)
